...
void file_fprint_simple_test() {
	...
	readSize = bctbx_file_read(fp, out_buf, F_SIZE, readSize); // offset a readSize, so we start reading at the end of what we read before
...
void file_fprint_and_write_test() {
	...
	BC_ASSERT_TRUE(memcmp(out_buf + strlen(patterns[0]), patterns[1]+strlen(patterns[0]), strlen(patterns[1]) - strlen(patterns[0])) == 0);
