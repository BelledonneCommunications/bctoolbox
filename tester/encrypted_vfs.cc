...
static void set_dummy_encryption_info(VfsEncryption &settings,
                                      size_t chunk_size) {
  const std::vector<uint8_t> keyMaterial{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
static void set_aes256_encryption_info(VfsEncryption &settings,
                                       size_t chunk_size) {
  const std::vector<uint8_t> keyMaterial{
...
EncryptedVfsOpenCb set_encryption_info = [](VfsEncryption &settings) {
  ...
  if (filename.find(bctoolbox::encryptionSuiteString(
          bctoolbox::EncryptionSuite::plain)) != std::string::npos) {
    set_plain_encryption_info(settings);
  } else if (filename.find(bctoolbox::encryptionSuiteString(
                 bctoolbox::EncryptionSuite::aes256gcm128_sha256)) !=
             std::string::npos) {
    set_aes256_encryption_info(settings, bctbx_vfs_tester_chunk_size);
  } else if (filename.find(bctoolbox::encryptionSuiteString(
                 bctoolbox::EncryptionSuite::dummy)) != std::string::npos) {
...
static const uint8_t message[256] = {
    0x42, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
 ...
 */
void basic_encryption_test(bctoolbox::EncryptionSuite suite,
                           bool closeFile = true) {
  ...
  /* create the file */
  bctbx_vfs_file_t *fp =
      bctbx_file_open2(&bcEncryptedVfs, filePath.data(), O_RDWR | O_CREAT);

  uint8_t readBuffer[256];
  memset(readBuffer, 0, sizeof(readBuffer));

  /* Make simple write, size is < block size and check we get back what we just
   * wrote */
  ...
  memset(readBuffer, 0, sizeof(readBuffer));
  BC_ASSERT_EQUAL(bctbx_file_read(fp, readBuffer, 32, 0), 4, ssize_t,
                  "%ld"); // try to read more than we have
  BC_ASSERT_TRUE(memcmp(readBuffer, message, 4) == 0);
  memset(readBuffer, 0, sizeof(readBuffer));
  ...
  BC_ASSERT_EQUAL(bctbx_file_read(fp, readBuffer, 16, 8), 8, ssize_t,
                  "%ld"); // read at index not 8, we have only 8 bytes to get
                          // ask for 16 anyway
  BC_ASSERT_TRUE(memcmp(readBuffer, message, 8) == 0);
  memset(readBuffer, 0, sizeof(readBuffer));
  ...
  BC_ASSERT_EQUAL(
      bctbx_file_read(fp, readBuffer, 16, 8), 0, ssize_t,
  ...
  if (suite == bctoolbox::EncryptionSuite::plain) {
    ...
    BC_ASSERT_EQUAL(bctbx_file_size(fp), 80, int64_t, "%ld");
    // read the part left there from offset 15 to 31
    BC_ASSERT_EQUAL(bctbx_file_read(fp, readBuffer, 16, 15), 16, ssize_t,
                    "%ld");
    BC_ASSERT_TRUE(memcmp(readBuffer, message, 16) == 0);
    memset(readBuffer, 0, sizeof(readBuffer));
    // read the part we just wrote
    BC_ASSERT_EQUAL(bctbx_file_read(fp, readBuffer, 18, 31), 18, ssize_t,
                    "%ld");
    BC_ASSERT_TRUE(memcmp(readBuffer, message, 18) == 0);
    memset(readBuffer, 0, sizeof(readBuffer));
    // read the part left after
    BC_ASSERT_EQUAL(bctbx_file_read(fp, readBuffer, 80, 49), 31, ssize_t,
                    "%ld");
    BC_ASSERT_TRUE(memcmp(readBuffer, message + 34, 31) == 0);
    memset(readBuffer, 0, sizeof(readBuffer));
...
void fprintf_encryption_test(bctoolbox::EncryptionSuite suite, int charsNb) {
  ...
  while (inSize < BCTBX_VFS_PRINTF_PAGE_SIZE) {
    sprintf(line,
            "this is a line used to fill the first page in the fprintf cache, "
            "it is the write number %04x, make it "
            "long so we have fewer fprintf to do to fill the cache, then we "
            "should one by one write %d chars\n",
            inSize, charsNb);
    memcpy(
        inBuf + inSize, line,
        strlen(
            line)); // build a buffer image of what we are writing in the file
    BC_ASSERT_TRUE(bctbx_file_fprintf(fp, 0, "%s", line) > 0);
    inSize += strlen(line);
  }
  // write in a second page, it will be written at file's close(unless it is
  // bigger than BCTBX_VFS_PRINTF_PAGE_SIZE)
  memset(line, 0x24, charsNb);
  line[charsNb] = '\0';
  memset(inBuf + inSize, 0x24, charsNb); // keep updated the buffer image of
                                         // what we are writing in the file
  BC_ASSERT_TRUE(bctbx_file_fprintf(fp, 0, "%s", line) > 0);
  inSize += charsNb;
  bctbx_file_close(fp);
  fp = NULL;
  ...
  BC_ASSERT_EQUAL(bctbx_file_read(fp, outBuf, inSize, 0), inSize, ssize_t,
                  "%ld");
...
void auth_fail_test(bctoolbox::EncryptionSuite suite) {
  ...
  BC_ASSERT_EQUAL(bctbx_file_read(fp, readBuffer, sizeof(message), 8),
                  sizeof(message), ssize_t, "%ld");
  ...
  file.seekg(30); // base header file is 29 bytes, at 30 we are in the integrity
                  // data of the encryption suite, change one byte
  char tweakBuf[2];
...
void migration_test(bctoolbox::EncryptionSuite suite) {
  ...
  bctbx_vfs_file_t *fp = bctbx_file_open2(bctbx_vfs_get_standard(),
                                          filePath.data(), O_RDWR | O_CREAT);
...
void recovery_test(bctoolbox::EncryptionSuite suite) {
  ...
  // base header file is 29, dummy module adds 16 bytes, aes256gcm128 adds 48
  // bytes
  std::fstream file(filePath, std::ios::in | std::ios::binary);
  char fileHeader[48 + 29];
  auto fileHeaderSize =
      (suite == bctoolbox::EncryptionSuite::dummy) ? (29 + 16) : (29 + 48);
  ...
  // Open it directly and rewrite the header as it was before to simulate an
  // error occuring before the call to writeHeader but after the file
  // modification So header is still a valid one but the size won't match
