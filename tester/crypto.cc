...
static void PBKDF2_HMAC_SHA_256_test(void) {
    ...
    /* Test vectors for PBKDF2-SHA1 from RFC 6070 */
    /* Test 1.1 */
    password = "password";
    salt = "salt";
    c = 1;
    dkLen = 20;
    res = PBKDF2<SHA1>(password, salt, c, dkLen);
    excp = {0x0c, 0x60, 0xc8, 0x0f, 0x96, 0x1f, 0x0e, 0x71,
    ...
    dkLen = 20;
    res = PBKDF2<SHA1>(password, salt, c, dkLen);
    excp = {0xea, 0x6c, 0x01, 0x4d, 0xc7, 0x2d, 0x6f, 0x8c,
    ...
    dkLen = 20;
    res = PBKDF2<SHA1>(password, salt, c, dkLen);
    excp = {0x4b, 0x00, 0x79, 0x01, 0xb7, 0x65, 0x48, 0x9a,
    ...
    dkLen = 25;
    res = PBKDF2<SHA1>(password, salt, c, dkLen);
    excp = {0x3d, 0x2e, 0xec, 0x4f, 0xe4, 0x1c, 0x84, 0x9b,
    ...
    c = 1;
    dkLen = 32;
    res = PBKDF2<SHA256>(password, salt, c, dkLen);
    excp = {0x12, 0x0f, 0xb6, 0xcf, 0xfc, 0xf8, 0xb3, 0x2c,
    ...
    dkLen = 64;
    res = PBKDF2<SHA256>(password, salt, c, dkLen);
    excp = {0x55, 0xac, 0x04, 0x6e, 0x56, 0xe3, 0x08, 0x9f, 0xec, 0x16, 0x91, 0xc2, 0x25, 0x44, 0xb6, 0x05,
    ...
    dkLen = 64;
    res = PBKDF2<SHA256>(password, salt, c, dkLen);
    excp = {0x4d, 0xdc, 0xd8, 0xf6, 0x0b, 0x98, 0xbe, 0x21, 0x83, 0x0c, 0xee, 0x5e, 0xf2, 0x27, 0x01, 0xf9,
    ...
    dkLen = 20;
    res = PBKDF2<SHA256>(password, salt, c, dkLen);
    excp = {0x12, 0x0f, 0xb6, 0xcf, 0xfc, 0xf8, 0xb3, 0x2c, 0x43, 0xe7, 0x22, 0x52, 0x56, 0xc4, 0xf8, 0x37, 0xa8, 0x65, 0x48, 0xc9};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 20;
    res = PBKDF2<SHA256>(password, salt, c, dkLen);
    excp = {0xae, 0x4d, 0x0c, 0x95, 0xaf, 0x6b, 0x46, 0xd3, 0x2d, 0x0a, 0xdf, 0xf9, 0x28, 0xf0, 0x6d, 0xd0, 0x2a, 0x30, 0x3f, 0x8e};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 20;
    res = PBKDF2<SHA256>(password, salt, c, dkLen);
    excp = {0xc5, 0xe4, 0x78, 0xd5, 0x92, 0x88, 0xc8, 0x41, 0xaa, 0x53, 0x0d, 0xb6, 0x84, 0x5c, 0x4c, 0x8d, 0x96, 0x28, 0x93, 0xa0};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 25;
    res = PBKDF2<SHA256>(password, salt, c, dkLen);
    excp = {0x34, 0x8c, 0x89, 0xdb, 0xcb, 0xd3, 0x2b, 0x2f, 0x32, 0xd8, 0x14, 0xb8, 0x11, 0x6e, 0x84, 0xcf, 0x2b, 0x17, 0x34, 0x7e, 0xbc, 0x18, 0x00, 0x18, 0x1c};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 16;
    res = PBKDF2<SHA256>(password, salt, c, dkLen);
    excp = {0x89, 0xb6, 0x9d, 0x05, 0x16, 0xf8, 0x29, 0x89, 0x3c, 0x69, 0x62, 0x26, 0x65, 0x0a, 0x86, 0x87};
    BC_ASSERT_TRUE(excp == res);
    ...
    c = 1;
    dkLen = 20;
    res = PBKDF2<SHA384>(password, salt, c, dkLen);
    excp = {0xc0, 0xe1, 0x4f, 0x06, 0xe4, 0x9e, 0x32, 0xd7, 0x3f, 0x9f, 0x52, 0xdd, 0xf1, 0xd0, 0xc5, 0xc7, 0x19, 0x16, 0x09, 0x23};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 20;
    res = PBKDF2<SHA384>(password, salt, c, dkLen);
    excp = {0x54, 0xf7, 0x75, 0xc6, 0xd7, 0x90, 0xf2, 0x19, 0x30, 0x45, 0x91, 0x62, 0xfc, 0x53, 0x5d, 0xbf, 0x04, 0xa9, 0x39, 0x18};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 20;
    res = PBKDF2<SHA384>(password, salt, c, dkLen);
    excp = {0x55, 0x97, 0x26, 0xbe, 0x38, 0xdb, 0x12, 0x5b, 0xc8, 0x5e, 0xd7, 0x89, 0x5f, 0x6e, 0x3c, 0xf5, 0x74, 0xc7, 0xa0, 0x1c};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 25;
    res = PBKDF2<SHA384>(password, salt, c, dkLen);
    excp = {0x81, 0x91, 0x43, 0xad, 0x66, 0xdf, 0x9a, 0x55, 0x25, 0x59, 0xb9, 0xe1, 0x31, 0xc5, 0x2a, 0xe6, 0xc5, 0xc1, 0xb0, 0xee, 0xd1, 0x8f, 0x4d, 0x28, 0x3b};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 16;
    res = PBKDF2<SHA384>(password, salt, c, dkLen);
    excp = {0xa3, 0xf0, 0x0a, 0xc8, 0x65, 0x7e, 0x09, 0x5f, 0x8e, 0x08, 0x23, 0xd2, 0x32, 0xfc, 0x60, 0xb3};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 128;
    res = PBKDF2<SHA384>(password, salt, c, dkLen);
    excp = {0x11, 0xc1, 0x98, 0x98, 0x77, 0x30, 0xfa, 0x11, 0x34, 0x58, 0x05, 0x3c, 0xd5, 0xcc, 0x9b, 0x51,
    ...
    c = 1;
    dkLen = 20;
    res = PBKDF2<SHA512>(password, salt, c, dkLen);
    excp = {0x86, 0x7f, 0x70, 0xcf, 0x1a, 0xde, 0x02, 0xcf, 0xf3, 0x75, 0x25, 0x99, 0xa3, 0xa5, 0x3d, 0xc4, 0xaf, 0x34, 0xc7, 0xa6};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 20;
    res = PBKDF2<SHA512>(password, salt, c, dkLen);
    excp = {0xe1, 0xd9, 0xc1, 0x6a, 0xa6, 0x81, 0x70, 0x8a, 0x45, 0xf5, 0xc7, 0xc4, 0xe2, 0x15, 0xce, 0xb6, 0x6e, 0x01, 0x1a, 0x2e};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 20;
    res = PBKDF2<SHA512>(password, salt, c, dkLen);
    excp = {0xd1, 0x97, 0xb1, 0xb3, 0x3d, 0xb0, 0x14, 0x3e, 0x01, 0x8b, 0x12, 0xf3, 0xd1, 0xd1, 0x47, 0x9e, 0x6c, 0xde, 0xbd, 0xcc};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 25;
    res = PBKDF2<SHA512>(password, salt, c, dkLen);
    excp = {0x8c, 0x05, 0x11, 0xf4, 0xc6, 0xe5, 0x97, 0xc6, 0xac, 0x63, 0x15, 0xd8, 0xf0, 0x36, 0x2e, 0x22, 0x5f, 0x3c, 0x50, 0x14, 0x95, 0xba, 0x23, 0xb8, 0x68};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 16;
    res = PBKDF2<SHA512>(password, salt, c, dkLen);
    excp = {0x9d, 0x9e, 0x9c, 0x4c, 0xd2, 0x1f, 0xe4, 0xbe, 0x24, 0xd5, 0xb8, 0x24, 0x4c, 0x75, 0x96, 0x65};
    BC_ASSERT_TRUE(excp == res);
    ...
    dkLen = 128;
    res = PBKDF2<SHA512>(password, salt, c, dkLen);
    excp = {0xe6, 0x33, 0x7d, 0x6f, 0xbe, 0xb6, 0x45, 0xc7, 0x94, 0xd4, 0xa9, 0xb5, 0xb7, 0x5b, 0x7b, 0x30,
