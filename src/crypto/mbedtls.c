...
struct bctbx_ssl_context_struct {
	...
	int(*callback_cli_cert_function)(void *, bctbx_ssl_context_t *, bctbx_list_t *); /**< pointer to the callback called to update client certificate during handshake
	...
	while( ssl_ctx->ssl_ctx.state != MBEDTLS_SSL_HANDSHAKE_OVER )
	{
		...
		if (ssl_ctx->callback_cli_cert_function != NULL) { /* check we have a callback function */
			...
			if (ssl_ctx->ssl_ctx.state == MBEDTLS_SSL_CLIENT_CERTIFICATE && ssl_ctx->ssl_ctx.client_auth > 0) {
				...
				const mbedtls_x509_crt* peer_cert = ssl_ctx->ssl_ctx.session_negotiate->peer_cert;
				bctbx_list_t *names = NULL;

				if (peer_cert != NULL) {
					/* This code is mostly copied from mbedtls function x509_crt_verify_name (x509_crt.c) */
					/* parse subjectAltName if any */
					if( peer_cert->ext_types & MBEDTLS_X509_EXT_SUBJECT_ALT_NAME ) {
						const mbedtls_x509_sequence *cur = &(peer_cert->subject_alt_names);
						while (cur != NULL) {
							char *name = bctbx_malloc(cur->buf.len+1); /* +1 for the null termination */
							snprintf(name, cur->buf.len+1, "%s", (const char *)cur->buf.p);
							names = bctbx_list_append(names, name);
							cur = cur->next;
						}
					}

					/* Add Subject CN */
					const mbedtls_x509_name *subject = &(ssl_ctx->ssl_ctx.session_negotiate->peer_cert->subject);
					while (subject != NULL) { // Certificate should hold only one CN, but be permissive and parse several if they are in the certificate
						if( MBEDTLS_OID_CMP( MBEDTLS_OID_AT_CN, &subject->oid ) == 0 ) { // subject holds all the distinguished name in asn1 format, get the CN only
							char *name = bctbx_malloc(subject->val.len+1); /* +1 for the null termination */
							snprintf(name, subject->val.len+1, "%s", (const char *)(subject->val.p));
							names = bctbx_list_append(names, name);
						}
...
int32_t bctbx_ssl_config_set_callback_cli_cert(bctbx_ssl_config_t *ssl_config, int(*callback_function)(void *, bctbx_ssl_context_t *, bctbx_list_t *), void *callback_data) {
	if (ssl_config == NULL) {
		return BCTBX_ERROR_INVALID_SSL_CONFIG;
	}
