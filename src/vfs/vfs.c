...
static bctbx_vfs_t *pDefaultVfs =
    &bcStandardVfs; /* bcStandardVfs is defined int vfs_standard.h*/
...
ssize_t bctbx_file_write(bctbx_vfs_file_t *pFile, const void *buf, size_t count,
                         off_t offset) {
  ssize_t ret;

  if (pFile != NULL) {
    if (bctbx_file_flush(pFile) <
        0) { // make sure our write is not overwritten by a page flush
...
ssize_t bctbx_file_write2(bctbx_vfs_file_t *pFile, const void *buf,
                          size_t count) {
...
static int file_open(bctbx_vfs_t *pVfs, bctbx_vfs_file_t *pFile,
                     const char *fName, const int oflags) {
...
bctbx_vfs_file_t *bctbx_file_open(bctbx_vfs_t *pVfs, const char *fName,
                                  const char *mode) {
  int ret;
  bctbx_vfs_file_t *p_ret =
      (bctbx_vfs_file_t *)bctbx_malloc(sizeof(bctbx_vfs_file_t));
...
bctbx_vfs_file_t *bctbx_file_open2(bctbx_vfs_t *pVfs, const char *fName,
                                   const int openFlags) {
  int ret;
  bctbx_vfs_file_t *p_ret =
      (bctbx_vfs_file_t *)bctbx_malloc(sizeof(bctbx_vfs_file_t));
...
static ssize_t bctbx_file_flush(bctbx_vfs_file_t *pFile) {
  ...
  size_t fSize = pFile->fSize; // save the size so we could restore it if
                               // something goes wrong
  pFile->fSize = 0; // set it to 0 now so when we call write it won't enter in
                    // an infinite loop(as write will call flush)
...
ssize_t bctbx_file_read(bctbx_vfs_file_t *pFile, void *buf, size_t count,
                        off_t offset) {
  int ret = BCTBX_VFS_ERROR;
  if (pFile) {
...
int bctbx_file_close(bctbx_vfs_file_t *pFile) {
  ...
  if (pFile) {
    ...
    /* clean the fprint and getline cache as they might hold the plain version
    ...
    if (ret != 0) {
      bctbx_error("bctbx_file_close: Error %s freeing file handle anyway",
                  strerror(-(ret)));
    }
  }
  bctbx_free(pFile);
...
int64_t bctbx_file_size(bctbx_vfs_file_t *pFile) {
  ...
  if (pFile) {
    ...
    if (ret < 0)
      bctbx_error("bctbx_file_size: Error file size %s", strerror((int)-(ret)));
  }
  return ret;
}
...
int bctbx_file_truncate(bctbx_vfs_file_t *pFile, int64_t size) {
  ...
  if (pFile) {
    ...
    if (ret < 0)
      bctbx_error("bctbx_file_truncate: Error truncate  %s",
                  strerror((int)-(ret)));
  }
  return ret;
}

ssize_t bctbx_file_fprintf(bctbx_vfs_file_t *pFile, off_t offset,
                           const char *fmt, ...) {
  char *ret = NULL;
  va_list args;
  ...
  if (ret != NULL) {
    ...
    if (count + pFile->fSize <
        BCTBX_VFS_PRINTF_PAGE_SIZE) { // Data fits in current page
    ...
    } else if (pFile->fSize > 0) { // There is a cache but the new data won't
                                   // fit in : write the cache and the new data
      char *buf = bctbx_malloc(
          count + pFile->fSize); // allocate a temporary buffer, to store the
                                 // current cache and the data to be written
      if (buf == NULL) {
        bctbx_free(ret);
        return BCTBX_VFS_ERROR;
      }
      memcpy(buf, pFile->fPage,
             pFile->fSize); // concatenate the cache and new data
      memcpy(buf + pFile->fSize, ret, count);
      bctbx_free(ret);

      // We are flushing the cache along the new data to write
      size_t fSizeBkp = pFile->fSize; // save the size so we could restore it if
                                      // something goes wrong
      pFile->fSize = 0; // cache empty: so the write try to flush it as we are
                        // already doing it
      r = bctbx_file_write(pFile, buf, fSizeBkp + count,
                           pFile->fPageOffset); // write all
      bctbx_free(buf);
      if (r < 0) {
...
/**
 * This function manages the offset stored in pFile and used by the
 * bctbx_file_get_nxtline function It has no effect on the current offset in the
 * underlying file which we have no use of
 */
...
static char *findNextLine(const char *buf) {
  ...
  if ((pNextLineR != NULL) && (pNextLineN != NULL))
    pNextLine = MIN(pNextLineR, pNextLineN);
  else if (pNextLineR != NULL)
    pNextLine = pNextLineR;
  else if (pNextLineN != NULL)
    pNextLine = pNextLineN;
  return pNextLine;
}
 ...
 * Reads at most max_len characters from the file descriptor associated with the
 * argument pFile and looks for an end of line character (\r or \n). Stores the
 * line found into the buffer pointed by s. Modifies the open file offset using
 * pFile->offset.
 *
 ...
 */
static int bctbx_generic_get_nxtline(bctbx_vfs_file_t *pFile, char *s,
                                     int max_len) {
  int64_t ret;
  int sizeofline;
  char *pNextLine = NULL;
  ...
  if ((pFile->gSize > 0) && (pFile->gPageOffset <= pFile->offset) &&
    ...
    if (pNextLine) {
      ...
      sizeofline =
          (int)(pNextLine - c + 1); // 1 one so it actually includes the \n char
      pFile->offset += sizeofline;  // offset to next beginning of line
      if ((pNextLine[0] == '\r') &&
          (pNextLine[1] ==
           '\n')) { // take into account the \r\n case, this case can pass
                    // underdetected if page ends in between, not a problem we
                    // will just get an extra empty line
        pFile->offset += 1;
      }
      memcpy(s, c, sizeofline - 1); // copy all before the \n
      s[sizeofline - 1] = '\0';     // add a \0 at the end where the \n was
      return sizeofline; // return size including the termination, so an empty
                         // line returns 1 (0 is for EOF)
    } else {             // No end of line found, did we reach the EOF?
      if (pFile->gPage[pFile->gSize - 1] ==
          0x04) { // 0x04 is EOT in ASCII, put in cache to signal the end of
                  // file
        sizeofline = (int)(pFile->gSize - (pFile->offset - pFile->gPageOffset) -
                           1); // size does not include the EOT char(which is
                               // part of the page size). so -1
        pFile->offset += sizeofline; // offset now points on the EOT char
        memcpy(s, c, sizeofline);    // copy everything before the EOT
        s[sizeofline] = '\0';
        return sizeofline; // Can be 0 if we were already pointing on EOT when
                           // we enter this call
      }
    }
  }
  ...
  if (ret > 0) {
    ...
    if (max_len - 1 < BCTBX_VFS_GETLINE_PAGE_SIZE) {
      ...
      if (ret <
          max_len - 1) { // read did not return as much as asked, we reached EOF
        pFile->gPage[readSize] = 0x04; // 0x04 is ASCII for EOT, use it to store
                                       // the EOF in the buffer
        pFile->gSize += 1;             // the EOT is part of the cached page
      }
      pFile->gPage[pFile->gSize] = '\0';
    } else {
      bctbx_warning("bctbx_get_nxtline given a max size value %d bigger than "
                    "cache size (%d), please adjust one or the other",
...
void bctbx_vfs_set_default(bctbx_vfs_t *my_vfs) { pDefaultVfs = my_vfs; }

bctbx_vfs_t *bctbx_vfs_get_default(void) { return pDefaultVfs; }

bctbx_vfs_t *bctbx_vfs_get_standard(void) { return &bcStandardVfs; }
