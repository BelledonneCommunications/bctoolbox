...
// someone does include the evil windef.h so we must undef the min and max
// macros to be able to use std::min and std::max
#undef min
#undef max
...
// is called at file creation when the encryption suite is set using
// encryptionSuiteSet
static std::shared_ptr<VfsEncryptionModule>
make_VfsEncryptionModule(const EncryptionSuite suite) {
  switch (suite) {
  ...
  default:
    throw EVFS_EXCEPTION << "Encrypted FS: unsupported encryption scheme "
                         << static_cast<uint16_t>(suite);
  }
}
// is called when an encrypted file header is parsed
static std::shared_ptr<VfsEncryptionModule>
make_VfsEncryptionModule(const uint16_t suite,
...
const std::string
bctoolbox::encryptionSuiteString(const EncryptionSuite suite) noexcept {
  switch (suite) {
  case EncryptionSuite::dummy:
    return "dummy";
  case EncryptionSuite::aes256gcm128_sha256:
 ...
 *    - magicnumber: hexadecimal ASCII for bcEncryptedFs:
 * 0x6263456E637279707465644673 : 13 bytes
 *    - version Number: 0xMMmm : 2 bytes
 *    - encryption suite: 2 bytes
 *    - chunk size : 2 bytes : number of 16 bytes blocks in a file chunk -
 * excluding chunk header if any.
 *    - Header extension size: 2 bytes. Flexibility on header size - so older
 * version of parser may be able to read newer versions
 *    - size: clear text file size : 8 bytes.
 *    - [Optionnal Header Extension]
 *    - [Optionnal Encryption module data - size is given by the encryption
 * suite selected]
 *
 * base header size is 29 bytes
 */
static const std::vector<uint8_t> BCENCRYPTEDFS = {0x62, 0x63, 0x45, 0x6e, 0x63,
                                                   0x72, 0x79, 0x70, 0x74, 0x65,
                                                   0x64, 0x46, 0x73};
...
VfsEncryption::VfsEncryption(bctbx_vfs_file_t *stdFp,
                             const std::string &filename, int openFlags,
                             int accessMode)
    : mVersionNumber(
          BcEncFS_v0100), // default version number is the current one
      mChunkSize(0),      // set to 0 at creation, is will be populated by
                     // parseHeader if there is one. If we are creating a file,
                     // let a chance to the callback to set the chunk size.
      m_module(nullptr), // encryption module is set by callback or when parsing
                         // the header
      mHeaderExtensionSize(0), mFilename(filename), mFileSize(0),
      mEncryptExistingPlainFile(false), mIntegrityFullCheck(false),
      mAccessMode(accessMode), pFileStd(stdFp) {

  if (stdFp == NULL)
    throw EVFS_EXCEPTION
        << "Cannot create a vfs encrytion object, vfs pointer is null";

  // If the file exists, read the header to check it is an encrypted file and
  // gets its encrypted policy if the file is plain, set the mFileSize so then
  // we now we already have a file but it is plain
  ...
  if (VfsEncryption::openCallbackGet() != nullptr) {
    ...
    throw EVFS_EXCEPTION
        << "Encrypted VFS: must provide a callback to setup key material";
  }

  if (m_module ==
      nullptr) { // this is a plain file and we want to keep it this way
    return;
  }

  /* check we have a valid chunk size */
  if (mChunkSize ==
      0) { // this is a file creation and the callback didn't set it
  ...
  if (mEncryptExistingPlainFile == true) { // we have a plain file to encrypt
    ...
    auto stdFdTmp = bctbx_file_open2(bctbx_vfs_get_standard(),
                                     tmpFilename.data(), O_WRONLY | O_CREAT);
    // read the whole file chunk by chunk and write their ciphertext to the temp
    // file
    ...
    do {
      // read
      auto readSize = bctbx_file_read(pFileStd, readBuf, mChunkSize,
                                      static_cast<off_t>(index));
      if (readSize < 0) {
      ...
      auto rawChunk = m_module->encryptChunk(
          currentChunkIndex, std::vector<uint8_t>(readBuf, readBuf + readSize));
      // write
      if (bctbx_file_write(stdFdTmp, rawChunk.data(), rawChunk.size(),
                           (off_t)getChunkOffset(currentChunkIndex)) -
              rawChunk.size() !=
          0) {
        bctbx_file_close(stdFdTmp);
        bctbx_free(readBuf);
    ...
    pFileStd =
        bctbx_file_open2(bctbx_vfs_get_standard(), mFilename.data(), openFlags);

  } else { // no migration but now we shall have all the material (settings and
           // keys ) to check the file integrity
    if (mFileSize > 0) { // this is not a file creation
      if (m_module->checkIntegrity(*this) != true) {
        throw EVFS_EXCEPTION << "Integrity check fail while opening file "
                             << mFilename;
      } else {                             // header integrity is Ok
        if (mIntegrityFullCheck == true) { // file size in header is wrong,
                                           // check each chunk and update header
          for (auto chunkIndex = getChunkIndex(mFileSize); chunkIndex > 0;
            ...
            ssize_t readSize =
                bctbx_file_read(pFileStd, rawData.data(), rawData.size(),
                                (off_t)getChunkOffset(chunkIndex));
            if (readSize >= 0) {
              rawData.resize(readSize);
            } else {
              throw EVFS_EXCEPTION << "fail to read file while trying to check "
                                      "the full integrity, file_read returned "
                                   << readSize;
            }

            std::vector<uint8_t> plainData(mChunkSize);

            // decrypt the chunk, if it fails it will generate an exception, let
            // it flow up
            m_module->decryptChunk(chunkIndex, rawData);
          }
          // all clear, update header
          writeHeader();
          BCTBX_SLOGW << "Encrypted FS: Whole file integrity check "
                         "successfull, update header with correct file size";
        }
      }
    }
  }
...
size_t VfsEncryption::chunkSizeGet() const noexcept {
  return (mChunkSize == 0
              ? defaultChunkSize
              : mChunkSize); // mChunkSize to 0 means it is not initialised yet,
                             // so return the default value in this case
};

/**
 * Set the size, in bytes, of chunks in which the file is divided for encryption
 * This size must be a multiple of 16.
 * If the size is set on an existing file and differs from previous setting, an
 * exception is generated Default chunk size at file creation is 4kB
 */
void VfsEncryption::chunkSizeSet(const size_t size) {
  ...
  if (mChunkSize == 0) {
    mChunkSize = size;
  } else { // chunk size is already set for this file, we cannot change it,
           // check the are the same
    if (mChunkSize != size) {
      throw EVFS_EXCEPTION << "Encrypted VFS to set chunk size " << size
                           << " on file " << mFilename << " but already set to "
                           << mChunkSize;
    }
  }
}
...
/**
 * Set a callback called during file opening to get the encryption material and
 * suite
 */
...
/**
 * Get the callback called during file opening to get the encryption material
 * and suite
 */
 ...
 */
void VfsEncryption::secretMaterialSet(
    const std::vector<uint8_t> &secretMaterial) {
  if (m_module == nullptr) {
    if (mFileSize > 0 &&
        mAccessMode == O_RDONLY) { // we are opening a read only plain file,
                                   // ignore the secret material
      BCTBX_SLOGW << " Encrypted VFS access a plain file " << mFilename
                  << "as read only. Secret material setting ignored";
      return;
    } else {
      throw EVFS_EXCEPTION << "Cannot set secret material before specifying "
                              "which encryption suite to use. file "
                           << mFilename;
    }
  }
...
void VfsEncryption::encryptionSuiteSet(const EncryptionSuite suite) {
  ...
  } else { // file already exists (if m_filesize!=0 and m_module is nullptr, it
           // is an existing plain file, the encryptionSuiteGet would return
           // plain)
    if (encryptionSuiteGet() != suite) {
      if (encryptionSuiteGet() ==
          bctoolbox::EncryptionSuite::plain) { // we want to migrate a plain
                                               // file to an encrypted one
        if (mAccessMode != O_RDONLY) {         // Do not migrate read-only file
          mEncryptExistingPlainFile = true;
          m_module = make_VfsEncryptionModule(suite);
        } else {
          BCTBX_SLOGW << " Encrypted VFS access a plain file " << mFilename
                      << "as read only. Kept it plain";
        }
      } else {
        throw EVFS_EXCEPTION << "Encryption suite for file " << mFilename
                             << " is already set to "
                             << encryptionSuiteString(encryptionSuiteGet())
                             << " but we're trying to set it to "
...
uint64_t VfsEncryption::rawFileSizeGet() const noexcept {
  ...
  return mFileSize                            // actual plain size
         ...
         + baseFileHeaderSize + mHeaderExtensionSize +
         m_module->getModuleFileHeaderSize(); // file header size
}
 ...
 *    - version Number: 0xMMmm : 2 bytes
 *    - encryption suite: 2 bytes
 *    - chunk size : 2 bytes : size of a chunk payload in number of 16 bytes
 * blocks - excluding chunk header if any. -> maximum chunk size is 2^16-1*16 =
 * 1 MB - recommended is 4Kb (256 blocks)
 *    - Header extension size: 2 bytes. Flexibility on header size - this
 * version of parser may be able to read newer future versions
 *    - size: clear text file size : 8 bytes.
...
void VfsEncryption::parseHeader() {
  // check file exists
  int64_t ret =
      bctbx_file_size(pFileStd); // bctbx_file_size return a signed value...
  if (ret <
      baseFileHeaderSize) { // this is not an EVFS file, assume it is plain
    mFileSize =
        ret; // set file size so we know that the file exists and is plain
    m_module = nullptr;
    return;
  }
  uint64_t fileSize =
      ret; // turn it into an unsigned one after checking it is >0.

  // read the header
  r_header = std::vector<uint8_t>(baseFileHeaderSize);
  size_t index = 0;
  if (bctbx_file_read(pFileStd, r_header.data(), baseFileHeaderSize, 0) !=
      baseFileHeaderSize)
  ...
  if (!std::equal(BCENCRYPTEDFS.cbegin(), BCENCRYPTEDFS.cend(),
                  r_header.cbegin())) { // file is not starting with our magic
                                        // number, assume it is plain
    mFileSize =
        ret; // set file size so we know that the file exists and is plain
    m_module = nullptr;
    return;
  ...
  if (mVersionNumber != BcEncFS_v0100) {
    BCTBX_SLOGW << "Encrypted FS trying to open a file version "
                << mVersionNumber << " but supports up to " << BcEncFS_v0100
                << ", this may not work, proceed anyway";
  }
  index += 2;

  // get the encryption suite, check it is supported and instanciate the
  // matching module
  ...
  // check the header extension, not used in version 1.0 but use this field to
  // skip them if any
  ...
  if (encryptionModuleDataSize != 0) {
    if (bctbx_file_read(pFileStd, encryptionSuiteData.data(),
                        encryptionModuleDataSize,
                        (off_t)(baseFileHeaderSize + mHeaderExtensionSize)) -
            encryptionModuleDataSize !=
        0) {
      throw EVFS_EXCEPTION << "Encrypted FS: unable to read encryption scheme "
                              "data in file header";
    }
  }
  ...
  // If they do not match, check all chunks integrity and update the header.
  // Recovery from failure between write and header update at last
  // write/truncate
  if (rawFileSizeGet() != fileSize) {
    BCTBX_SLOGW << "Encrypted FS: meta data file size " << mFileSize
                << " and actual raw filesize " << fileSize
...
void VfsEncryption::writeHeader(bctbx_vfs_file_t *fp) {
  if (m_module == nullptr) {
    throw EVFS_EXCEPTION << "Encrypted VFS: cannot write file Header when no "
                            "encryption module is selected";
  }
  ...
  // moduleFileHeader shall depends on the file header as it probably authentify
  // it, so do this update before asking for the encryption module header
  r_header = header;

  // add encryption module data
  auto moduleFileHeader = m_module->getModuleFileHeader(*this);
  header.insert(header.end(), moduleFileHeader.cbegin(),
                moduleFileHeader.cend());

  // write header to file (to the object file pointer if none is given as
  // parameter)
  ssize_t ret = bctbx_file_write((fp == nullptr) ? pFileStd : fp, header.data(),
                                 header.size(), 0);
  if (ret - header.size() !=
      0) { // cannot compare directly signed and unsigned...
    throw EVFS_EXCEPTION << "Encrypted VFS: something went wrong while writing "
                            "file header. file_write returns "
...
size_t VfsEncryption::getChunkOffset(uint32_t index) const noexcept {
  return rawChunkSizeGet() * index // all previous chunks
         + baseFileHeaderSize + mHeaderExtensionSize +
         m_module->getModuleFileHeaderSize();
}
...
std::vector<uint8_t> VfsEncryption::read(size_t offset, size_t count) const {
  ...
  if (m_module == nullptr) {
    ...
    auto readSize =
        bctbx_file_read(pFileStd, plain.data(), plain.size(), (off_t)offset);
  ...
  uint32_t lastChunk = getChunkIndex(
      offset + count -
      1); // -1 as we read data from indexes offset to offset + count - 1
  size_t offsetInFirstChunk = offset % mChunkSize;

  // allocate a vector large enough to store all the data to read : number of
  // chunks * size of raw chunk(payload+header)
  std::vector<uint8_t> rawData((lastChunk - firstChunk + 1) *
                               rawChunkSizeGet());

  /* read all chunks from actual file */
  ssize_t readSize = bctbx_file_read(pFileStd, rawData.data(), rawData.size(),
                                     (off_t)getChunkOffset(firstChunk));

  /* resize rawData to the actual content size - last chunk may be incomplete */
  if (readSize >= 0) {
    rawData.resize(readSize);
  } else {
    throw EVFS_EXCEPTION << "fail to read file " << mFilename
                         << " file_read returned " << readSize;
  }
  ...
  while (rawData.size() > m_module->getChunkHeaderSize()) {
    std::vector<uint8_t> plainChunk = m_module->decryptChunk(
        firstChunk++,
        std::vector<uint8_t>(rawData.cbegin(),
                             rawData.cbegin() +
                                 std::min(rawChunkSizeGet(), rawData.size())));
    plainData.insert(plainData.end(), plainChunk.cbegin(), plainChunk.cend());
    // remove the decrypted chunk
    rawData.erase(rawData.begin(), rawData.begin() + std::min(rawChunkSizeGet(),
                                                              rawData.size()));
  }
  ...
  plainData.erase(
      plainData.begin(),
      plainData.begin() +
          std::min(offsetInFirstChunk,
                   plainData.size())); // remove unwanted chunk begining
...
size_t VfsEncryption::write(const std::vector<uint8_t> &plainData,
                            size_t offset) {
  // plain file?
  if (m_module == nullptr) {
    ssize_t ret = bctbx_file_write(pFileStd, plainData.data(), plainData.size(),
                                   (off_t)offset);
    if (ret - plainData.size() == 0) { // compare signed and unsigned
      return plainData.size();
    } else {
      throw EVFS_EXCEPTION << "plain file fail to write to physical file "
                           << ret;
    }
  }
  ...
  uint64_t finalFileSize = std::max(
      mFileSize,
      static_cast<decltype(mFileSize)>(
          plain.size() + offset)); // we might need to increase the file size
  ...
  uint32_t lastChunk = getChunkIndex(
      offset + plain.size() -
      1); // -1 as we write data from indexes offset to offset + data size - 1
  size_t rawDataSize =
      (lastChunk - firstChunk + 1) *
      rawChunkSizeGet(); // maximum size used, last chunk might be incomplete
  std::vector<uint8_t>
      rawData{}; // Store the existing encrypted chunks with header that are
  ...
  size_t readOffset =
      offset -
      offset %
          mChunkSize; // we must start read/write at the begining of a chunk

  if (readOffset < mFileSize) { // Yes we are overwritting some data, read all
                                // the existing chunks we are overwritting
    rawData.resize(rawDataSize);
    ssize_t overwrittenSize =
        bctbx_file_read(pFileStd, rawData.data(), rawDataSize,
                        (off_t)getChunkOffset(firstChunk));
  ...
  if (readOffset <
      offset) { // we need to get the plain data from readOffset to offset
    ...
    auto plainChunk = m_module->decryptChunk(
        ...
        std::vector<uint8_t>(rawData.cbegin(),
                             rawData.cbegin() +
                                 std::min(rawChunkSizeGet(), rawData.size())));
    plain.insert(
        plain.begin(), plainChunk.cbegin(),
        plainChunk.cbegin() +
            (offset - readOffset)); // prepend the begining to our plain buffer
  }

  // append the plain buffer if needed:
  if ((plain.size() % mChunkSize != 0) &&
      (plain.size() + readOffset <
       mFileSize)) { // We do not have an integer number of chunks to write and
                     // we have data after our last written byte
    auto plainChunk = m_module->decryptChunk(
        lastChunk,
        std::vector<uint8_t>(rawData.cbegin() + getChunkOffset(lastChunk) -
                                 getChunkOffset(firstChunk),
                             rawData.cbegin() +
                                 std::min(getChunkOffset(lastChunk + 1) -
                                              getChunkOffset(firstChunk),
                                          rawData.size())));
    plain.insert(
        plain.end(), plainChunk.cbegin() + (plain.size() % mChunkSize),
  ...
  while (rawData.size() > 0) {
    ...
    std::vector<uint8_t> rawChunk(
        rawData.cbegin(),
        rawData.cbegin() + std::min(rawChunkSizeGet(), rawData.size()));
    // delete it
    rawData.erase(rawData.begin(), rawData.begin() + std::min(rawChunkSizeGet(),
                                                              rawData.size()));
    // re-encrypt
    m_module->encryptChunk(
        ...
        std::vector<uint8_t>(plain.cbegin(),
                             plain.cbegin() +
                                 std::min(mChunkSize, plain.size())));
    // delete consumed plain
    plain.erase(plain.begin(),
                plain.begin() + std::min(mChunkSize, plain.size()));
    // store the result
    updatedRawData.insert(updatedRawData.end(), rawChunk.cbegin(),
                          rawChunk.cend());
  }
  ...
  ssize_t ret =
      bctbx_file_write(pFileStd, updatedRawData.data(), updatedRawData.size(),
                       (off_t)getChunkOffset(firstChunk));
  if (ret - updatedRawData.size() == 0) { // compare signed and unsigned
    ...
    throw EVFS_EXCEPTION << "fail to write to physical file " << mFilename
                         << " file_write " << ret;
  }
}
...
void VfsEncryption::truncate(const uint64_t newSize) {
  ...
  if (mFileSize < newSize) {
    write(std::vector<uint8_t>{},
          static_cast<size_t>(newSize)); // write nothing at new size index, the
                                         // gap is filled with 0 by write
    return;
  }
  ...
  if (mFileSize > newSize) {
    ...
    if (newSize % mChunkSize != 0) {
      ...
      if (bctbx_file_write(pFileStd, rawData.data(), rawData.size(),
              ...
              rawData.size() !=
          0) {
        throw EVFS_EXCEPTION << "Cannot write file " << mFilename
                             << " during truncate";
      }
    }
...
std::string VfsEncryption::filenameGet() const noexcept { return mFilename; }
 ...
 * @return         		BCTBX_VFS_ERROR if an error occurs, BCTBX_VFS_OK
 * otherwise.
 */
static int bcOpen(bctbx_vfs_t *pVfs, bctbx_vfs_file_t *pFile, const char *fName,
                  int openFlags);
 ...
 */
static ssize_t bcRead(bctbx_vfs_file_t *pFile, void *buf, size_t count,
                      off_t offset) {
  if (pFile && pFile->pUserData) {
    VfsEncryption *ctx = static_cast<VfsEncryption *>(pFile->pUserData);
    ...
    } catch (EvfsException const
                 &e) { // cannot let raise an exception to a C context
      BCTBX_SLOGE << "Encrypted VFS: error while reading " << count
                  << " bytes from file " << ctx->filenameGet() << " at offset "
                  << offset << ". " << e;
    }
  }
  return BCTBX_VFS_ERROR;
 ...
 * @return         number of bytes written (can be 0), negative value errno if
 * an error occurred.
 */
static ssize_t bcWrite(bctbx_vfs_file_t *pFile, const void *buf, size_t count,
                       off_t offset) {
  if (offset < 0)
    return BCTBX_VFS_ERROR;
  if (pFile && pFile->pUserData) {
    VfsEncryption *ctx = static_cast<VfsEncryption *>(pFile->pUserData);
    return (ssize_t)ctx->write(
 ...
 ** Truncate a file
 * @param pFile File handle pointer.
 * @param new_size Extends the file with null bytes if it is superior to the
 *file's size truncates the file otherwise.
 * @return -errno if an error occurred, 0 otherwise.
 */
static int bcTruncate(bctbx_vfs_file_t *pFile, int64_t new_size) {

  if (new_size < 0)
    return BCTBX_VFS_ERROR; // vfs interface force usage of int64_t as new
                            // size...
  int ret = BCTBX_VFS_ERROR;
...
static bool_t bcIsEncrypted(bctbx_vfs_file_t *pFile) {
  if (pFile && pFile->pUserData) {
    VfsEncryption *ctx = static_cast<VfsEncryption *>(pFile->pUserData);
    return (ctx->encryptionSuiteGet() == bctoolbox::EncryptionSuite::plain)
               ? FALSE
               : TRUE;
  }
  return FALSE;
...
static int bcOpen(BCTBX_UNUSED(bctbx_vfs_t *pVfs), bctbx_vfs_file_t *pFile,
                  const char *fName, int openFlags) {
  ...
  try {
    ...
    if ((filename.size() > 8) &&
      ...
      // This is a journal file use debug trace level
      BCTBX_SLOGD << "[EVFS]("
                  << encryptionSuiteString(ctx->encryptionSuiteGet())
                  << ") open " << filename;
    } else {
      BCTBX_SLOGI << "[EVFS]("
                  << encryptionSuiteString(ctx->encryptionSuiteGet())
  ...
  } catch (EvfsException const &e) { // caller is most likely a C file(vfs.c),
                                     // so swallow all exceptions
    if (stdFp != nullptr) {
